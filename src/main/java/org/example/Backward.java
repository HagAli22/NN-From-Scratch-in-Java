...
public class Backward {
    ...
    public void Backpropagation(Forward forward, Loss loss) {
     ...
     * @param Y_batch  Ground truth labels [m x output_size]
     * @param W1, W2, W3  Current weights
     * @param b1, b2, b3  Current biases
     */
    public void backward(double[][] X_batch, double[][] Y_batch,
        ...
        // ================= Hidden Layer 2 =================
        double[][] W3_T = Matrix_Operations.transpose(W3);
        double[][] dA2 = Matrix_Operations.multiply(dZ3, W3_T);

        // Apply ReLU derivative (placeholder - replace when z2 derivative ready)
        ...
        for (int i = 0; i < dA2.length; i++)
            for (int j = 0; j < dA2[0].length; j++)
                dZ2[i][j] = dA2[i][j] /* * Activation_Function.reluDerivative(net2[i][j]) */;
        ...
        // ================= Hidden Layer 1 =================
        double[][] W2_T = Matrix_Operations.transpose(W2);
        double[][] dA1 = Matrix_Operations.multiply(dZ2, W2_T);

        double[][] dZ1 = new double[dA1.length][dA1[0].length];
        double[][] net1 = forward.net1;
        for (int i = 0; i < dA1.length; i++)
            for (int j = 0; j < dA1[0].length; j++)
                dZ1[i][j] = dA1[i][j] /* * Activation_Function.reluDerivative(net1[i][j]) */;
